...
- A WebAssembly conformance suite: eight spec-shaped module fixtures under
  `tests/fixtures/wasm/` (generated by `tools/gen_wasm_fixtures.py`) drive
  every numeric, memory, and control opcode family through the real
  parse → validate → instantiate → invoke path, including traps, NaN
  propagation and signed-zero semantics (15 tests).
- `src/media_budget.rs`, the single source of truth for media byte budgets,
  with compile-time invariants tying the video/audio split to the total.
- Media-budget regression tests (`tests/media_budget_test.rs`): oversized
  inputs are rejected at the streaming-decoder and backend input gates
  before any decode work, and pipeline acceptance matches the shared
  ceiling exactly.

### Fixed (spec conformance)

- The WASM interpreter's numeric tables matched the spec: `0x45` is
  `i32.eqz` (pops one value) and the i32/i64 comparison, arithmetic and
  conversion blocks no longer run shifted; `clz`/`ctz`/`popcnt`, integer
  division overflow traps (`INT_MIN / -1`), `f32/f64` `nearest`
  (round-ties-to-even), NaN-propagating `min`/`max`, `copysign`, and
  trapping float→int conversions replace Rust-saturating casts;
  `extend8/16/32_s` were added. Modules built by real toolchains now
  execute correctly.
- Memory loads apply the documented sign/zero extension per opcode
  (`load8_s` vs `load8_u` … `load32_s` vs `load32_u`) instead of a
  permuted mapping.
- Branching to the outermost block now continues execution after the
  block's `end` instead of returning from the whole function; branch
  targets are resolved by an immediate-aware scanner so constant bytes
  (e.g. `i32.const 11`) can no longer be mistaken for control opcodes.
- The structural validator accepts the `unreachable`, `nop`, `return`,
  `drop`, `select`, typed `select`, `memory.size` and `memory.grow`
  instructions the interpreter already implemented, and rejects the
  unimplemented `table.get`/`table.set` opcodes it previously let through.

### Changed

- Media decode budgets are unified at 64 MB of decoded bytes (48 MB video /
  16 MB audio) across the Windows Media Foundation backend, the page media
  pipeline and the streaming decoder; the backend no longer accepts 256 MB
  of decoded output that the pipeline would immediately discard. Budget
  errors now state the measured and allowed byte counts. MediaSource
  queues keep their distinct 256 MB ENCODED-byte ceiling.
- Release engineering reads the version from `Cargo.toml` only:
  `tools/personal-release-gate.ps1` takes an optional `-RequireVersion`
  pin instead of a hardcoded version, `packaging/GhitaBrowser-Setup.iss`
  fails to compile without `/DMyAppVersion`, and
  `PERSONAL_RELEASE_CHECKLIST.md` uses a `$VERSION` placeholder.
- The release gate raises `RUST_MIN_STACK` to 32 MB after rustc worker
  threads overflowed the previous 16 MB while compiling large integration
  test crates on low-RAM machines.
- README "Deliberate limitations" now states the actual status of the
  experimental modules (Picture-in-Picture, quick notes, password storage,
  Web Capture, sidebar apps, split-screen): they are unwired prototypes
  unreachable from the UI, and `notes.rs`/`passwords.rs` carry in-code
  disposition contracts (never persisted, never exposed, until the
  password store is OS-vault backed).
